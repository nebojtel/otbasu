...
/* OTBASU VITRINE PRODUCT CARD SIZE FIX — SAFE
   Делает карточки товаров одинаковой высоты.
   Высокие/длинные фото больше не растягивают карточку.
   Галерею, свайпы и админку не трогаем.
*/
(() => {
  if (window.__OTBASU_VITRINE_PRODUCT_CARD_SIZE_FIX__) return;
  window.__OTBASU_VITRINE_PRODUCT_CARD_SIZE_FIX__ = true;

  const STYLE_ID = 'otbasu-vitrine-product-card-size-fix-style';

  function injectProductCardSizeFix() {
    if (document.getElementById(STYLE_ID)) return;
    ...
    style.textContent = `
      /* Все карточки товаров одинаковые */
      ...
      /* Левая зона фото — фиксированная */
      .products-list .product .product-media,
      [data-products-list] .product .product-media,
      .products-list .product [data-gallery-open],
      [data-products-list] .product [data-gallery-open] {
        ...
        background:
          radial-gradient(circle at 50% 0%, rgba(255, 232, 200, .55), transparent 46%),
          rgba(255, 248, 239, .95) !important;
      }

      /* Фото внутри карточки не растягивает карточку */
      .products-list .product .product-media img,
      [data-products-list] .product .product-media img,
      .products-list .product [data-gallery-open] img,
      [data-products-list] .product [data-gallery-open] img {
        width: 100% !important;
        height: 100% !important;
      ...
      /* Правая часть карточки не должна растягивать товар */
      .products-list .product h2,
      .products-list .product h3,
      [data-products-list] .product h2,
      [data-products-list] .product h3 {
      ...
      [data-products-list] .product .actions {
  ...
  injectProductCardSizeFix();
  document.addEventListener('DOMContentLoaded', injectProductCardSizeFix);
  window.setTimeout(injectProductCardSizeFix, 500);
  window.setTimeout(injectProductCardSizeFix, 1500);
})();
