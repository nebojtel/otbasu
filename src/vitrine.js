...
bindEvents();
loadState();
/* OTBASU MOBILE PHOTO VIEWER — FINAL SAFE VERSION
   Влево/вправо — листать фото.
   Вверх/вниз — закрыть просмотр.
   CSS встроен сюда, vitrine/styles.css не трогаем.
*/
(() => {
  if (window.__OTBASU_MOBILE_PHOTO_VIEWER_FINAL__) return;
  window.__OTBASU_MOBILE_PHOTO_VIEWER_FINAL__ = true;

  const STYLE_ID = 'otbasu-mobile-photo-viewer-final-style';
  ...
  function openViewer(product, images, opener) {
    ...
    const resetVertical = () => {
      ...
      track.style.transition = 'transform 240ms cubic-bezier(.16,1,.3,1), opacity 200ms ease';
    ...
    const endTouch = (event) => {
      ...
      const fastVertical = mode === 'vertical' && ay > 42 && time < 240 && ay > ax;
    ...
    track.addEventListener('touchcancel', resetVertical, { passive: true });
